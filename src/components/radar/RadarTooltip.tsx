...
export const RadarTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white p-5 border rounded-lg shadow-lg max-w-[280px]">
        ...
        <div className="space-y-3">
          {payload.map((entry: any, index: number) => (
            <p key={index} className="flex items-center gap-3 text-sm" style={{ color: entry.color }}>
              <span className="w-4 h-4 rounded-full inline-block" style={{ backgroundColor: entry.color }}></span>
